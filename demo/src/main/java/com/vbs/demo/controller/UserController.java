...
public class UserController {
    ...
    public String update(@RequestBody UpdateDto obj) {
        ...
        {
            if (user.getName().equalsIgnoreCase(obj.getValue())) return "Name Cannot be same";
            h1.setDescription("User changed name from "+user.getName()+" to "+obj.getValue());
            user.setName(obj.getValue());
        }
        else if (obj.getKey().equalsIgnoreCase("password"))
        {
            if (user.getPassword().equalsIgnoreCase(obj.getValue())) return "Password Cannot be same";
            h1.setDescription("User changed Password: "+user.getUsername());
            user.setPassword(obj.getValue());
        }
        else if (obj.getKey().equalsIgnoreCase("email"))
        {
            if (user.getEmail().equalsIgnoreCase(obj.getValue())) return "Email Cannot be same";
